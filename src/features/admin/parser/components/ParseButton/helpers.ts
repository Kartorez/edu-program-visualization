...
import { normalizeCode as sharedNormalizeCode } from '@/shared/lib/normalize';

export const normalizeCode = sharedNormalizeCode;
...
export async function resolveOrCreate(
  ...
  const key = `${collection}:${normCode}`;
  if (cache.has(key)) return cache.get(key)!;

  const res = await fetch(
    `/api/${collection}?where[code][equals]=${encodeURIComponent(normCode)}&limit=1`
  );
