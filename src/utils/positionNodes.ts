...
export function positionNodes(rawNodes: Disciplines): Node<Discipline>[] {
  ...
  years.forEach((year) => {
    ...
    const s1nodes = rawNodes.filter((n) =>
      n.semesters.includes(semester1)
    ).length;

    const s2nodes = rawNodes.filter((n) =>
      n.semesters.includes(semester2)
    ).length;

    const width = (Math.max(s1nodes, s2nodes) + 1) * COL_WIDTH + PADDING;
  ...
  rawNodes.forEach((node) => {
    node.semesters.forEach((semester) => {
      if (!(semester in semesterCounters)) {
        ...
        const nodesInSemester = rawNodes.filter((n) =>
          n.semesters.includes(semester)
        ).length;

        const width = (nodesInSemester + 1) * COL_WIDTH + PADDING;
        ...
        result.push({
          ...
          position: {
            x: PADDING,
            y: getSemesterY(semester) + PADDING,
          },
          style: {
            height: ROW_HEIGHT,
            width,
